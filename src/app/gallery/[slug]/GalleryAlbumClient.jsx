...
export default function GalleryAlbumClient({ album, slug }) {
  ...
  const prevImage = useCallback(() => {
    setLightboxIndex((i) => {
      ...
      setIntrinsic(null);
      return i > 0 ? i - 1 : album.images.length - 1;
    });
  }, [album?.images?.length]);
  ...
  const nextImage = useCallback(() => {
    setLightboxIndex((i) => {
      if (i === null || !album?.images?.length) return i;
      setIntrinsic(null);
      return i < album.images.length - 1 ? i + 1 : 0;
    });
  }, [album?.images?.length]);
  ...
  // Prefetch adjacent full-res images so next/prev appears instantly
  useEffect(() => {
    if (lightboxIndex === null || !album?.images?.length) return;
    const preload = (i) => {
      const src = album.images[(i + album.images.length) % album.images.length]?.full;
      if (src) { const img = new window.Image(); img.src = src; }
    };
    preload(lightboxIndex + 1);
    preload(lightboxIndex - 1);
  }, [lightboxIndex, album?.images]);
  ...
  const computeDisplaySize = () => {
    ...
    return { width: Math.floor(iw * scale) + "px", height: Math.floor(ih * scale) + "px" };
  };
      ...
      <main className="relative min-h-screen overflow-hidden bg-[#0b0b10] text-neutral-100">
        ...
        <div className="mx-auto max-w-7xl px-4 pt-28 pb-16">
          ...
          <p className="mt-2 text-sm text-neutral-300">
            {album.name ? `A selection from ${album.name}.` : ""}
          </p>
