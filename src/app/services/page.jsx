...
const ss2 = "/images/ss2.png";
const s2 = "/images/s2.png";
const s3 = "/images/s3.png";
...
const Hero = () => (
  <section className="relative isolate overflow-hidden bg-[#0b0b10] text-neutral-100">
    <Image src={backgroundImage} alt="" fill priority quality={80} className="object-cover object-center -z-20" />
    ...
    <div className="mx-auto grid max-w-7xl grid-cols-1 items-stretch px-6 py-24 md:grid-cols-2 md:py-32 lg:px-12">
      ...
      <motion.div {...fade(0.1)} className="relative mt-12 md:mt-0">
        <div className="relative aspect-[4/3] w-full overflow-hidden rounded-3xl ring-1 ring-white/10 shadow-[0_10px_40px_rgba(0,0,0,0.45)]">
          <Image src={ss2} alt="Elegant event" fill className="object-cover" loading="lazy" quality={80} sizes="(min-width: 768px) 50vw, 100vw" />
    ...
    <div className="pointer-events-none absolute inset-0 -z-30 opacity-[0.08] [background-image:radial-gradient(currentColor_1px,transparent_1px)] [background-size:6px_6px] text-neutral-400" />
...
const ServiceCard = ({ img, title, blurb, index }) => (
  <motion.div {...fade(0.05 * index)} className="group relative overflow-hidden rounded-3xl ring-1 ring-white/10 bg-white/5">
    <div className="relative aspect-[4/3] w-full overflow-hidden">
      <Image src={img} alt={title} fill className="object-cover transition duration-500 group-hover:scale-[1.04]" loading="lazy" quality={80} sizes="(min-width: 1024px) 33vw, (min-width: 768px) 50vw, 100vw" />
      <div className="absolute inset-0 bg-gradient-to-t from-black/55 via-black/10 to-transparent" />
    </div>
    <div className="p-6">
      <h3 className="font-serif text-xl font-semibold tracking-tight">{title}</h3>
      <p className="mt-2 text-sm leading-relaxed text-neutral-300">{blurb}</p>
    </div>
  </motion.div>
);
    ...
    <div className="mx-auto max-w-7xl px-6 lg:px-12">
      <motion.div {...fade(0)} className="text-center">
        <h2 className="font-serif text-3xl md:text-4xl font-bold tracking-tight">
          <span className="bg-gradient-to-r from-amber-200 via-rose-200 to-amber-100 bg-clip-text text-transparent">Our Services</span>
        </h2>
        <p className="mx-auto mt-4 max-w-2xl text-neutral-300">Choose the partnership that suits your journey—from full planning to styling & day‑of orchestration.</p>
      </motion.div>
...
const ProcessStep = ({ n, title, text }) => (
  <div className="relative flex gap-4">
    <div className="mt-1 flex h-8 w-8 shrink-0 items-center justify-center rounded-full border border-amber-300/60 bg-white/5 text-sm font-semibold text-amber-100">{n}</div>
    <div>
      <div className="font-medium text-neutral-100">{title}</div>
      <p className="text-sm text-neutral-300/90">{text}</p>
    </div>
  </div>
);

const Process = () => (
  <section className="relative bg-[#0b0b10] py-20">
    <div className="mx-auto grid max-w-7xl grid-cols-1 items-center gap-10 px-6 lg:grid-cols-2 lg:px-12">
      <motion.div {...fade(0)}>
        <h2 className="font-serif text-3xl md:text-4xl font-bold tracking-tight">
          <span className="bg-gradient-to-r from-amber-200 via-rose-200 to-amber-100 bg-clip-text text-transparent">How It Works</span>
        </h2>
        <p className="mt-4 max-w-prose text-neutral-300">A calm, collaborative process with clear milestones and meticulous communication.</p>
      ...
      <motion.div {...fade(0.1)} className="relative">
        <div className="relative aspect-[4/3] w-full overflow-hidden rounded-3xl ring-1 ring-white/10 bg-white/5">
          <Image src={ss2} alt="Process visual" fill className="object-cover" loading="lazy" quality={80} sizes="(min-width: 1024px) 50vw, 100vw" />
...
const Quote = () => (
  <section className="relative bg-[#0b0b10] py-16">
    <div className="mx-auto max-w-4xl px-6 text-center lg:px-12">
      <motion.blockquote {...fade(0)} className="rounded-3xl border border-white/10 bg-white/5 p-8 backdrop-blur-sm">
        <p className="text-lg leading-relaxed text-neutral-200">"From the first call, we felt completely taken care of. The design was breathtaking and the day flowed effortlessly."</p>
        <footer className="mt-3 text-sm text-neutral-400">— H&M, Aqaba</footer>
      </motion.blockquote>
    </div>
  </section>
);
  ...
  <section className="relative overflow-hidden bg-[#0b0b10] py-20">
    ...
    <div className="mx-auto max-w-7xl px-6 text-center lg:px-12">
      <motion.h3 {...fade(0)} className="font-serif text-2xl md:text-3xl font-bold tracking-tight">
        <span className="bg-gradient-to-r from-amber-200 via-rose-200 to-amber-100 bg-clip-text text-transparent">Ready to begin?</span>
      </motion.h3>
