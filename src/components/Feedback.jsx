...
function Feedback({ autoRotate = true, intervalMs = 5000 }) {
  const testimonials = useMemo(
    () => [
      {
        message: "Thank you from the bottom of our hearts for bringing our vision to life so beautifully. The night was beyond perfect-every detail, big and small, was thoughtful.",
        author: "Dana & fares",
        rating: 5,
      },
      {
        message: '"Thank you" feels far too small for what you did for us. Our wedding was truly everything we dreamed of and more, and we are so deeply grateful to you for making it all possible.',
        author: "Diala and Yazan",
        rating: 5,
      },
      {
        message: "Thank you for the incredible work you put into our wedding. The setup was absolutely beautiful!! The dance floor with the catwalk wooowww especially with the fountain fireworks.",
        author: "Dina and Ahmad",
        rating: 5,
      },
      {
        message: "Thank you for your professionalism, good taste, and making this full wedding come together better than expected. You listened to every request and detail and made it a reality.",
    ...
    >
      ...
      <div className="hidden sm:flex items-center justify-center gap-6 sm:gap-8 w-full max-w-4xl">
        ...
        <div
          ...
          aria-atomic="true"
        >
          <div className="mb-3 flex items-center justify-center gap-1 text-amber-300 text-lg" aria-label={`${t.rating} out of 5 stars`}>
            {Array.from({ length: 5 }).map((_, i) => (
              <span key={i} aria-hidden="true">{i < t.rating ? "★" : "☆"}</span>
            ))}
            <span className="sr-only">{t.rating} out of 5</span>
          </div>

          <p className={`mx-auto max-w-3xl text-base sm:text-lg text-neutral-100/95 mb-4 transition-transform duration-300 ${animate ? "translate-x-[-40px] opacity-0" : "translate-x-0 opacity-100"}`}>
            {t.message}
          </p>

          <p className={`font-bold text-lg sm:text-xl text-white transition-transform duration-300 ${animate ? "translate-x-[40px] opacity-0" : "translate-x-0 opacity-100"}`}>
            {t.author}
          </p>

          {autoRotate && (
            <div className="mt-6 h-1 w-full rounded bg-white/10">
              <div ref={progressRef} className="h-1 w-0 rounded bg-gradient-to-r from-amber-300 via-rose-300 to-amber-200" aria-hidden="true" />
            </div>
          )}
        </div>
      ...
      <div className="sm:hidden w-full max-w-xl">
        <div
          ...
          aria-atomic="true"
        >
          <div className="mb-2 flex items-center justify-center gap-1 text-amber-300 text-base" aria-label={`${t.rating} out of 5 stars`}>
            {Array.from({ length: 5 }).map((_, i) => (
              <span key={i} aria-hidden="true">{i < t.rating ? "★" : "☆"}</span>
            ))}
            <span className="sr-only">{t.rating} out of 5</span>
          ...
          <p className={`text-base mb-3 text-neutral-100/95 transition-transform duration-300 ${animate ? "translate-x-[-40px] opacity-0" : "translate-x-0 opacity-100"}`}>
            {t.message}
          </p>

          <p className={`font-bold text-lg text-white transition-transform duration-300 ${animate ? "translate-x-[40px] opacity-0" : "translate-x-0 opacity-100"}`}>
            {t.author}
          </p>
          ...
          {testimonials.map((_, i) => (
            <button
              ...
              className={`h-2.5 w-2.5 rounded-full transition ${
                i === current ? "bg-amber-300 shadow-[0_0_0_4px_rgba(251,191,36,0.25)]" : "bg-white/20 hover:bg-white/40"
              }`}
            />
          ))}
        </div>
      ...
      <div className="mt-6 hidden sm:flex items-center justify-center gap-2">
        {testimonials.map((_, i) => (
          <button
            ...
            onClick={() => setCurrent(i)}
            className={`h-2.5 w-2.5 rounded-full transition ${
              i === current ? "bg-amber-300 shadow-[0_0_0_6px_rgba(251,191,36,0.18)]" : "bg-white/20 hover:bg-white/40"
            }`}
          />
        ))}
      </div>
